...
from typing import Dict, List, Tuple, Iterable, Optional, Any, Callable, Set
...
# failures.
SWMM_IMPORT_ERROR: Optional[Exception]
...
try:  # pragma: no cover - only executed at import time
    ...
    swmmtoolbox = None  # type: ignore[assignment]
    _tsutils = None  # type: ignore[assignment]
    SWMM_IMPORT_ERROR = exc
else:  # pragma: no cover - import-time bookkeeping
    SWMM_IMPORT_ERROR = None


def require_swmmtoolbox() -> None:
    """Raise a helpful error when the ``swmmtoolbox`` dependency is missing."""

    if swmmtoolbox is None:  # pragma: no cover - exercised via subprocess tests
        raise ImportError(
...
class FilenameTemplateError(ValueError):
    """Raised when a user-supplied filename template cannot be formatted."""

    def __init__(self, pattern: str, reason: str, *, available: Iterable[str]):
        available_sorted = ", ".join(sorted(available)) or "(none)"
        if "unexpected '{'" in reason:
            reason = (
                f"{reason}. Escape literal braces as '{{{{' or '}}}}'."
            )
        message = (
            f"Invalid filename template {pattern!r}: {reason}. "
            f"Available fields: {available_sorted}."
        )
...
def render_filename_template(
    ...
    """Format ``pattern`` (or ``default`` when empty) with ``context`` safely."""

    template = pattern or default
    try:
...
_SAFE_LABEL_TOKEN = re.compile(r"[^0-9A-Za-z#_-]+")


def collapse_label_token(label: str) -> str:
    """Return ``label`` stripped of unsafe characters without adding delimiters."""

    token = _SAFE_LABEL_TOKEN.sub("", label or "")
    return token or "series"


def _outfile_components(outfile: str) -> Tuple[str, str, str]:
    """Return ``(abs_path, base, parent_safe)`` for ``outfile``."""

    abs_path = os.path.abspath(outfile)
    base = os.path.splitext(os.path.basename(abs_path))[0]
    parent = os.path.basename(os.path.dirname(abs_path)) or "root"
    parent_safe = sanitize_id(parent) or "root"
    return abs_path, base, parent_safe


def output_subdir_base(outfile: str) -> str:
    """Return the human-friendly directory name for ``outfile`` outputs."""

    _, base, parent_safe = _outfile_components(outfile)
    return f"{base}__{parent_safe}"


def output_subdir_name(outfile: str) -> str:
    """Return the legacy hashed directory name for ``outfile`` outputs."""

    abs_path, base, parent_safe = _outfile_components(outfile)
    digest = hashlib.sha1(abs_path.encode("utf-8")).hexdigest()[:8]
    return f"{base}__{parent_safe}_{digest}"


def resolve_output_subdirs(
    filelist: Iterable[str], output_dir: Optional[str]
) -> Dict[str, str]:
    """Return per-file output subdirectories, adding hashes only when needed."""

    resolved: Dict[str, str] = {}
    taken_by_root: Dict[str, Set[str]] = defaultdict(set)
    existing_by_root: Dict[str, Set[str]] = {}

    for outfile in filelist:
        abs_path, _, _ = _outfile_components(outfile)
        root = os.path.abspath(output_dir) if output_dir else os.path.dirname(abs_path)

        existing = existing_by_root.get(root)
        if existing is None:
            try:
                entries = [
                    name
                ...
                entries = []
            existing = existing_by_root[root] = set(entries)

        base_name = output_subdir_base(outfile)
        candidate = base_name
        if candidate in existing or candidate in taken_by_root[root]:
            candidate = output_subdir_name(outfile)

        resolved[outfile] = candidate
        taken_by_root[root].add(candidate)

    return resolved


def find_duplicate_basenames(paths: Iterable[str]) -> Dict[str, List[str]]:
    """Return a mapping of basenames that appear more than once in ``paths``."""

    grouped: Dict[str, List[str]] = defaultdict(list)
    for p in paths:
        base = os.path.splitext(os.path.basename(p))[0]
        grouped[base].append(p)
    return {base: files for base, files in grouped.items() if len(files) > 1}
...
def build_output_name(pattern: str, out_format: str, **kwargs) -> str:
    ...
    are preserved.
    """
    base = render_filename_template(pattern, "{prefix}{short}{id}{suffix}", kwargs)
    m = re.search(r"\.([^./]+)$", base)
...
def plan_elements(outfile: str,
                  ...
                  out_subdir: Optional[str] = None) -> List[str]:
    """Return the filenames that would be written for this selection (no IO)."""
    planned: List[str] = []
    subdir = out_subdir or output_subdir_name(outfile)
    out_dir = os.path.join(outdir_root, subdir)
    if item_type == "system":
        element_ids = ["SYSTEM"]
    for elem_id in element_ids:
        if combine_mode == "com":
            if out_format == "tsf":
                fname = render_filename_template(
                    tsf_template_com,
                    "{prefix}{type}{id}{suffix}.tsf",
                    {
                        "prefix": prefix,
                        "type": item_type,
                ...
                combined_short = "".join(param_short.get(p, p) for p in params)
                pattern = dat_template or "{prefix}{type}{id}{suffix}"
            ...
            for p in params:
                if out_format == "tsf":
                    fname = render_filename_template(
                        tsf_template_sep,
                        "{prefix}{type}{id}{param}{suffix}.tsf",
                        {
                            "prefix": prefix,
                            "type": item_type,
                            "id": sanitize_id(elem_id),
                    ...
                    pattern = dat_template or "{prefix}{short}{id}{suffix}"
                    short = param_short.get(p, p)
    ...
    for elem_id in element_ids:
        ...
        if combine_mode == "com":
            ...
            if out_format == "tsf":
                ...
                for df, lab, _ in frames[1:]:
                    left = left.join(df.rename(columns={"value": lab}), how="outer")
                combined_short = "".join(param_short.get(p, p) for _, _, p in frames)
                pattern = dat_template or "{prefix}{type}{id}{suffix}"
                fname = build_output_name(
                    pattern,
                    out_format,
            ...
            for df, lab, p in frames:
                if out_format == "tsf":
                    ...
                    fname = build_output_name(pattern, out_format, prefix=prefix,
                                               ...
                                               type=item_type, param=p)
                    fpath = os.path.join(out_dir, fname)
                    param_sep = ',' if out_format == 'csv' else '\t'
                    header = f"IDs:{param_sep}{elem_id}\n" \
                             f"Date/Time{param_sep}{param_short.get(p, p)}"
                    if out_format == "csv":
...
def combine_across_files(
    ...
    for (item_type, elem_id, label), paths in buckets.items():
        ...
        short = collapse_label_token(label)
        if out_format == "tsf":
            pattern = tsf_template_sep or "{prefix}{type}{id}{param}{suffix}"
        else:
            pattern = dat_template or "{prefix}{short}{id}{suffix}"
        fname = build_output_name(
            pattern,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "DAT pattern. Add separators manually if desired, e.g. "
            "'{prefix}_{short}_{id}{suffix}.dat'"
        ),
    )
    p.add_argument(
        "--tsf-template-sep",
        default="",
        help=(
            "TSF per-param pattern. Add separators manually if desired, e.g. "
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "TSF combined pattern. Add separators manually if desired, e.g. "
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.output_dir:
        duplicates = find_duplicate_basenames(filelist)
        if duplicates:
            details = "; ".join(
                f"{base}: {', '.join(files)}" for base, files in duplicates.items()
            )
            logging.warning(
                "Multiple input files share basenames (%s). Outputs will be written "
                "to unique subdirectories within %s.",
                details,
                args.output_dir,
            )

    subdir_map = resolve_output_subdirs(filelist, args.output_dir)
